...
import type { ValidationRule, ValidationGroup, UISchema } from "@screamform/core/domain/schema/types";
import { evaluateValidation, validateFormData } from "@screamform/core/domain/schema/validator";
...
describe("Domain: Validator Engine", () => {
  ...
  describe("Base Operators (Leaves)", () => {
    test("required: should detect empty values", () => {
      const rule: ValidationRule = { type: "required", errorMessage: "Field is mandatory" };
    ...
    test("regex: should validate email formats", () => {
      const rule: ValidationRule = { type: "regex", value: "^\\S+@\\S+\\.\\S+$", errorMessage: "Bad Email" };
    ...
    test("in: should validate against allowed set", () => {
      const rule: ValidationRule = { type: "in", value: ["USD", "EUR"], errorMessage: "Unsupported Currency" };
    ...
    test("startsWith: should validate string prefix", () => {
      const rule: ValidationRule = { type: "startsWith", value: "DOC_", errorMessage: "Must start with DOC_" };
      expect(evaluateValidation(rule, "DOC_final.pdf")).toBeNull();
      expect(evaluateValidation(rule, "final.pdf")).toBe("Must start with DOC_");
    });

    test("endsWith: should validate string suffix", () => {
      const rule: ValidationRule = { type: "endsWith", value: ".pdf", errorMessage: "Must end with .pdf" };
    ...
    test("min/max: should validate numeric bounds", () => {
      const minRule: ValidationRule = { type: "min", value: 10, errorMessage: "Too low" };
      expect(evaluateValidation(minRule, 15)).toBeNull();
      expect(evaluateValidation(minRule, 5)).toBe("Too low");
      const maxRule: ValidationRule = { type: "max", value: 100, errorMessage: "Too high" };
    ...
    test("contains: should validate array or string contains value", () => {
      const rule: ValidationRule = { type: "contains", value: "ok", errorMessage: "Must contain ok" };
  ...
  describe("Recursive Logic (Groups)", () => {
    ...
    test("OR: should return 'None of the required conditions were met' when group has no rules", () => {
      ...
      expect(evaluateValidation(group, "anything")).toBe("None of the required conditions were met");
    });
    ...
    test("NOT: should negate the result", () => {
      const group: ValidationGroup = {
        operator: "not",
        rules: [
          { type: "contains", value: "admin", errorMessage: "No admins allowed" }
        ]
      };
      ...
      expect(evaluateValidation(group, "admin-123")).toBe("This value is specifically disallowed");
    });
  });
  ...
  describe("Fallback Logic", () => {
    test("should use DEFAULT_VALIDATION_MESSAGES if errorMessage is missing", () => {
      ...
      expect(evaluateValidation(rule, "")).toBe(DEFAULT_VALIDATION_MESSAGES.required);
    });
  ...
  describe("validateFormData", () => {
    ...
    test("should skip fields without validation", () => {
      const schema: UISchema = {
        fields: {
          name: { label: "Name", widget: "text", validation: { type: "required", errorMessage: "Required" } },
